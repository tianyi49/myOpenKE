...
from openke.module.model import TransE, TransR,TransD,BertTransE,MBertTransR,AttMBertTransR
from openke.module.model import TransH
from openke.module.model import RESCAL
from openke.module.loss import MarginLoss
from openke.module.strategy import NegativeSampling,AttNegativeSampling
...
def train_mberttransr():
    ...
    transe.save_parameters(IN_PATH+"result/MBertTransr_transe.json")
    # train transr
...
def train_att_mberttransr():
    ...
    trainer.run()
    parameters = transe.get_parameters()
    transe.save_parameters(IN_PATH+"result/AttMBertTransR_transe.json")
    # train transr

    transr.set_parameters(parameters)
    trainer = myTrainer(model=model_r, data_loader=train_dataloader, train_times=100, alpha=1.0, use_gpu=True)
    trainer.run()
    transr.save_checkpoint(IN_PATH+'checkpoint/AttMBertTransR.ckpt')
    print("#####################MBertTransR Test##########################")
    # test the model
