...
class myTester(object):
    ...
    def run_link_prediction(self, type_constrain = False,kgcFlag = False):
        self.lib.initTest()
        ...
        if(kgcFlag):
            ...
            for index, [data_head, data_tail] in enumerate(training_range):
                ...
                if int(data_tail['batch_h'][0]) not in kgc_dct:
                    kgc_dct[int(data_tail['batch_h'][0])] = {}
                if int(data_tail['batch_r'][0]) not in kgc_dct[int(data_tail['batch_h'][0])]:
                    kgc_dct[int(data_tail['batch_h'][0])][int(data_tail['batch_r'][0])] =[]
                # 使用 np.argsort() 函数获取数组中元素的排序索引
                indices = np.argsort(score)
                # 获取排序后数组中最小的 10 个元素的索引
                min_indices = indices[:10]
                # 获取对应的最小元素值
                min_elements = score[min_indices]
                for i in range(len(min_indices)):
                    kgc_dct[int(data_tail['batch_h'][0])][int(data_tail['batch_r'][0])].append((int(min_indices[i]),float(min_elements[i])))
            return kgc_dct
        else:
            for index, [data_head, data_tail] in enumerate(training_range):
